...
为避免“任意主体 × 任意客体都可配动作”导致语义失真，平台应引入**动作类型签名**：
...
2. \(T_O\)：客体类型集合。  
3. \(A\)：动作集合。  
4. \(\Sigma_A\)：允许配置与推导的主体类型、客体类型、动作三元组白名单。
...
1. 不在 \(\Sigma_A\) 的三元组视为“类型上不可定义”，不得进入策略求值。  
2. 租户扩展动作能力时，先扩展签名，再写策略规则。  
3. 推导规则与委派规则不得生成签名外动作事实。

示例说明（与你提出的问题一致）：

1. 若未声明 `(company, kb, read)`，则“公司主体读取知识库”在模型层非法。  
2. 业务需要时可显式注册该三元组，再由策略决定 allow/deny。

### 6.2 签名闭包与委派边界（新增）

为防止委派链条越权，委派动作需满足双重约束：

1. 被委派动作集合 `delegated_actions ⊆ delegator_effective_actions`。  
2. `delegated_actions` 中每个动作三元组均在 \(\Sigma_A\) 内。

若违反任一约束，必须阻断配置发布。

### 6.3 关系端点签名（新增）

为避免“关系类型可绑定任意端点”导致语义漂移，平台应引入 `relation_signature`，将关系合法性收敛为“关系类型 × 端点类型签名”：

1. `subject_relations`：约束 `subject -> subject` 的 `relation_type/from_type/to_type`。  
2. `object_relations`：约束 `object -> object` 的 `relation_type/from_type/to_type`。  
3. `subject_object_relations`：约束 `subject -> object` 的 `relation_type/from_type/to_type`。
...
1. 关系边写入时，`from/to` 实际类型必须命中对应签名元组。  
2. `relation_type` 已注册并不等于端点组合合法，仍需通过端点签名。  
3. 发布门禁 `P0` 默认阻断 `RELATION_SIGNATURE_MISMATCH`。
...
1. `not_applicable` 建议细分原因码：`no_matched_rule`、`signature_mismatch`。  
2. 高风险域对 `indeterminate` 必须 `fail-closed`（按 deny 处理）。
...
#### 10.4.8 同部门可见的自动推导语义（新增）

对于“同部门用户可见部门内知识库”这类高频语义，**禁止**在对象创建时做 `用户 × 对象` 的显式授权展开。  
同时，平台实现不得把 `department`、`same_department` 等业务概念硬编码在引擎里。  
正确做法是“**模型声明推导规则** + **关系事实最小同步** + **PDP 运行时推导**”：

1. 只同步稳定关系事实：  
   a. `user -> department`（`belongs_to`）  
   b. 对象归属：`department -> object`（`owns`）或 `object -> department`（`owned_by`）  
   c. 若暂未同步对象归属关系，可使用 `object.owner_ref` 回溯到 owner 的部门关系作为兜底推导。
2. 模型通过 `context_inference.rules[]` 声明：  
   a. 用哪些关系类型与方向计算主体侧集合（`subject_edges`）  
   b. 用哪些关系类型与方向计算客体侧集合（`object_edges`）  
   c. 交集命中后写入哪个上下文字段（`output_field`）  
   d. 是否启用 `owner_ref` 兜底（`object_owner_fallback`）  
   e. `owner_ref` 兜底是否包含“当前输入对象”本身（`owner_fallback_include_input`）
3. 判权请求只需提供最小上下文（`subject.id`、`action`、`object.id`）。
4. PDP 在命中模型路由后，按模型规则推导并注入 `context.<output_field>`，再执行策略匹配。
5. 当主体关系、对象归属、owner 变化时，下一次判权自动生效，无需批量改策略。
...
`owner_fallback_include_input` 语义约定（必须明确）：

1. 仅在 `object_owner_fallback=true` 时生效。  
2. `true`（默认）：把“当前输入对象 owner”也纳入兜底推导。  
3. `false`：仅把“由 `object_edges` 路径扩展出的对象集合 owner”纳入兜底。  
4. 典型场景：  
   a. 直接对象可见（如 `same_company_direct`）通常设为 `true`；  
   b. 衍生对象沿 source 链可见（如 `same_company_via_derived_source`）通常设为 `false`，避免被输入对象 owner 干扰。

`owner_fallback_include_input` 配置决策表（给业务用户）：

| 你的业务语义 | `object_owner_fallback` | `owner_fallback_include_input` | 推荐说明 |
| --- | --- | --- | --- |
| “当前对象 owner 与用户同组织即可” | `true` | `true` | 最常见默认场景，直接按输入对象 owner 参与匹配 |
| “只看衍生来源对象的 owner，不看当前对象 owner” | `true` | `false` | 适用于 derived/source 链路可见，避免被衍生对象 owner 干扰 |
| “对象侧关系已完整，不需要 owner 兜底” | `false` | 不配置（或忽略） | 只走 `object_edges` 路径推导，语义最纯 |
| “不确定怎么选” | `true` | `true` | 先用默认值，按审计样本再收敛到 `false` |

业务同学三步法（不懂推导也能配）：

1. 先回答一句话：权限是否需要“看输入对象 owner”？  
2. 如果答案是“需要”，设 `object_owner_fallback=true`。  
3. 再判断是否“只看来源对象 owner”：是则 `owner_fallback_include_input=false`，否则 `true`。

该模式的治理收益：

1. 新建对象不触发 `N` 条用户授权写入，写放大显著降低。  
2. 策略数量与部门规模解耦，避免规则爆炸。  
3. 人员调岗或对象归属变更后权限自动收敛，降低残留越权风险。  
4. 与“持续关系事件同步 + 实时鉴权”链路天然一致。

推导边界（规范补充）：

1. `context_inference.rules` 推荐限制在单调规则子集，避免递归否定。  
2. 推导规则头部若产生动作事实，必须落在 `action_signature` 允许域。  
3. SoD/基数等治理约束不应混入推导递归，应在约束层独立校验。  
4. 跨租户推导默认禁止，租户边界属于平台强制约束而非策略可选项。

#### 10.4.9 Decision Search（主体可见对象检索）语义（新增）

`Decision Evaluate` 与 `Decision Search` 是两类能力，平台必须同时支持：

1. `Decision Evaluate`：输入 `subject + action + object`，返回单对象四值决策。  
2. `Decision Search`：输入 `subject + action + filters`，返回可见对象集合（分页）。
...
1. \(\delta\)：单对象四值决策函数。  
2. \(\varphi(o)\)：对象过滤谓词（类型、标签、敏感度等）。  
3. `Decision Search` 默认仅返回 `allow` 对象；`deny/not_applicable/indeterminate` 不入结果集。

推荐接口（当前实现命名）：

1. `POST /decisions:evaluate`：单次鉴权。  
2. `POST /decisions/search`：可见对象检索。

`Decision Search` 请求示意：
...
```json
{
  ...
  "input": {
    ...
    "context": { "request_time": "2026-03-05T10:00:00Z" }
  },
  "filters": {
    "object_type_in": ["kb"],
  ...
  "options": { "include_plan": true, "max_candidates_scan": 5000 }
}
```

`Decision Search` 响应最小字段：

1. `items[]`：对象ID、最终效果、命中规则、义务建议。  
2. `page`：`limit/has_more/next_cursor/total_count`。  
3. `resolved_model` 与 `resolved_route`：明确本次路由到哪个模型快照。  
4. `plan`（可选）：下推/残差执行计划与扫描统计。

平台约束：

1. **禁止**默认退化为“应用先拉全量对象，再逐个调用 `Decision Evaluate`”。  
2. 检索链路优先采用“集合计算 + 页内精评估”两阶段执行。  
3. 高风险动作在真正执行前仍应使用 `Decision Evaluate` 强校验（防陈旧越权）。

#### 10.4.10 Decision Search 执行规范（新增）

可下推判定（满足全部条件才可下推）：

1. 语法可编译：子句属于受控 DSL 子集（如 `==` / `includes` / `and`）。  
2. 数据可本地获得：依赖对象与关系已在控制面可查。  
3. 语义可等价：下推结果与逐条求值在四值语义上保持一致。  
4. 执行可索引：可映射为 `=` / `IN` / `EXISTS` / `JOIN`。  
5. 结果可解释：返回 `plan` 说明下推子句与残差子句。
6. 若无法证明强等价，可采用“保守候选超集 + 残差精评估”模式，保证不漏掉应允许对象。

数学判据（规范补充）：

\[
R_{full}=\{o\in O\mid \delta_{full}(s,a,o,c)=allow\}
\]

\[
R_{push}=\{o\in O\mid \delta_{push}(s,a,o,c)=allow\}
\]

1. `FullyPushdown` 需满足 \(R_{full}=R_{push}\)。  
2. `PushdownWithResidual` 需满足 \(R_{full}\subseteq C_{push}\)，再经残差求值得到最终 \(R_{full}\)。  
3. 若既无法证明等价，也无法证明保守超集，则必须降级 `EvaluateOnly` 或返回 `Reject`。

执行分层：

1. `FullyPushdown`：全部子句可下推，直接集合返回。  
2. `PushdownWithResidual`：先下推得到候选，再页内精评估残差。  
3. `EvaluateOnly`：不可安全下推，仅在小候选集上求值。  
4. `Reject`：候选失控或语义不安全，显式拒绝请求。

推荐流程：

1. `Route`：先命中单模型快照（同 10.4.7）。  
2. `Compile`：提取可下推子句（如 `object.type`、`object.sensitivity`）。  
3. `Pushdown`：按对象索引和关系索引筛候选。  
4. `Residual`：对页内对象执行完整策略求值。  
5. `Combine`：按 `deny-overrides` 输出最终集合与分页。

#### 10.4.11 Instance 持久化模式边界（新增）

企业可按系统成熟度选择 instance 接入模式；业务数据真源始终在业务系统，不在 ACL：

1. `external_snapshot`（仅模型持久化）  
   说明：业务方每次判权临时传入 `subject/object/context`，ACL 不保留对象实例索引。  
   适用：单次鉴权为主。  
   限制：难以高效支持 `Decision Search`。
2. `control_plane_index`（模型 + 实例索引）  
   说明：业务真源仍在业务库，ACL 只保存只读索引副本（对象/关系）用于检索与推导。  
   适用：需要高效列表检索与关系推导。  
   风险：存在同步延迟，需要治理 freshness 与回放语义。
3. `hybrid`（混合）  
   说明：列表走索引检索，高风险动作执行前再走点查强校验。  
   适用：既要检索效率又要关键动作强一致。

治理要求：

1. 明确“单一真源”原则：业务系统负责事实数据，ACL 不做业务主数据主库。  
2. 若启用索引模式，必须输出 `as_of`/快照时间和新鲜度指标。  
3. 同一 `namespace` 应固定接入模式，避免语义漂移与审计歧义。

#### 10.4.12 model 与 instance 混合判权场景（新增）

在 `hybrid` 模式下，推荐显式覆盖以下三类核心场景，并补充一个冲突场景：

1. 场景 A：`external_snapshot` 动态属性判权（无 instance 索引）  
   a. 系统仅持久化模型，不写入对象/关系 instance。  
   b. 调用方在 `Decision Evaluate` 时传入 `subject/object` 属性。  
   c. 策略直接基于请求属性求值，验证“无索引也可判权”。
2. 场景 B：`control_plane_index` 索引推导判权（有 instance 索引）  
   a. 系统维护对象与关系 instance 索引。  
   b. 请求仅传最小字段（`subject.id`、`object.id`、`action`，可选 type），不依赖业务侧重复传属性。  
   c. PDP 通过 `context_inference` + 索引数据推导上下文后完成判权。
3. 场景 C：model 内显式 instance 规则（高优先级定点授权）  
   a. 在 `policies.rules` 增加“主体 ID + 对象 ID”粒度规则（如 `subject.id == user:a` 且 `object.id == kb:1`）。  
   b. 该规则用于表达业务白名单/紧急放行。  
   c. 推荐使用 `ordered-deny-overrides` 并给该规则最高 `priority`，以确保其在冲突时按预期生效。

可选增强（建议作为第 4 场景）：

4. 场景 D：请求上下文与索引推导冲突  
   a. 请求显式传入 `context.same_company=false`。  
   b. `context_inference` 基于索引推导得到 `same_company=true`。  
   c. 验证最终求值使用推导后的上下文，避免调用方绕过关系事实。

推荐优先级语义（同一模型内）：

1. `policies.rules` 的冲突裁决始终由 `combining_algorithm + priority` 决定。  
2. “model 内 instance 规则优先”并非引擎硬编码特例，而是通过更高 `priority` + 合适合并算法达成。  
3. 若算法为 `deny-overrides`，即使 instance allow 规则优先级更高，也可能被任意命中的 deny 规则压制；因此该场景应优先选 `ordered-deny-overrides`。

建议测试断言（最小集）：

1. 场景 A：`relation_inference.enabled=false` 时仍能 `allow`。  
2. 场景 B：不传属性时，`relation_inference.rules[].matched=true` 且 `final_effect=allow`。  
3. 场景 C：同时命中 `deny` 与“instance allow”时，最终命中最高优先级规则。  
4. 场景 D：冲突字段以推导结果为准，并体现在决策上下文回传中。
...
| `catalogs.action_catalog` | list | 是 | 动作标识唯一、非空字符串 | 全量动作目录 |
| `catalogs.subject_type_catalog` | list | 是 | 主体类型唯一、非空字符串 | 全量主体类型目录 |
| `catalogs.object_type_catalog` | list | 是 | 客体类型唯一、非空字符串 | 全量客体类型目录 |
| `action_signature.tuples[]` | list | 是 | 每条含 `subject_types/object_types/actions` | 动作类型签名白名单 |
| `action_signature.tuples[].actions` | list | 是 | 每个动作必须在 `action_catalog` 已注册 | 签名动作集合 |
| `action_signature.tuples[].subject_types` | list | 是 | 每个类型必须在 `subject_type_catalog` 已注册 | 签名主体类型集合 |
| `action_signature.tuples[].object_types` | list | 是 | 每个类型必须在 `object_type_catalog` 已注册 | 签名客体类型集合 |
| `relations.*.relation_type` | enum | 是 | 在对应关系域 catalog（subject/object/subject_object）中已注册 | 关系类型 |
...
| `context_inference.rules[]` | list | 否 | 关系类型已注册且规则可静态类型检查 | 上下文推导规则 |
| `context_inference.rules[].object_owner_fallback` | bool | 否 | `true` 时启用 `owner_ref` 兜底推导 | owner 兜底开关 |
| `context_inference.rules[].owner_fallback_include_input` | bool | 否 | 仅在 `object_owner_fallback=true` 生效；默认 `true` | 是否把输入对象 owner 纳入兜底 |
| `context_inference.constraints.monotonic_only` | bool | 否 | 推荐为 `true` | 推导规则单调性约束 |
| `decision_search.pushdown.require_semantic_equivalence` | bool | 否 | `true` 时仅允许可证明等价下推 | 检索语义安全开关 |
| `decision_search.pushdown.allow_conservative_superset` | bool | 否 | 允许“候选超集 + 残差精评估” | 检索保守下推开关 |
...
1. `context_inference.rules` 推荐限制在可判定子集（纯正规则或分层否定）。  
2. 禁止递归否定与无界聚合（避免不可判定/不可解释）。  
3. 推导规则若生成动作事实，必须满足 `action_signature` 类型签名。  
4. 推导层不承载 SoD/基数等治理约束，治理约束由 `constraints` 独立表达。
...
3. 语义校验：选择器可解析、动作在目录内、关系类型合法、`action_signature` 不越界。  
4. 安全校验：SoD、高敏策略、越权传播、危险算法组合、`Decision Search` 下推语义安全。  
5. 可执行性校验：obligations 可执行、属性来源可达、一致性级别满足域要求、下推/残差计划可解释。
...
7. `动作签名越界`：规则命中的主体类型、客体类型、动作三元组不在 `action_signature`。  
8. `检索下推漂移`：`Decision Search` 下推谓词与完整求值语义不等价。
...
2. 展开动作包为原子动作集合。  
3. 基于 `action_signature` 生成动作类型签名索引。  
4. 将选择器与 `context_inference.rules` 编译为可执行谓词。  
5. 构建规则索引（按主体域、客体域、动作域、优先级）。  
6. 生成 `Decision Search` 执行计划（下推子句、残差子句、判据类型）。  
7. 生成可解释元数据（规则来源、覆盖链、证据键、推导路径）。
...
| `ACTION_SIGNATURE_MISMATCH` | error | 主体类型、客体类型、动作三元组不在 `action_signature` | 是 | 补充签名白名单或修正规则 |
| `RELATION_TYPE_UNKNOWN` | error | 使用未注册关系类型 | 是 | 先注册关系类型 |
| `INFERENCE_RULE_UNSAFE` | error | 推导规则含递归否定/无界聚合/不可判定结构 | 是 | 收敛到受控推导子集 |
...
| `SEARCH_PUSHDOWN_UNSAFE` | error | 检索下推无法证明等价且不满足保守超集条件 | 是 | 降级 `EvaluateOnly` 或修正下推规则 |
| `SEARCH_SEMANTIC_DRIFT` | warning | 检索结果与逐对象完整求值存在差异 | 视域而定 | 收紧下推并回放差异样本 |
...
| F14 | `subject_type/object_type/action` × `action_signature` | 三元组在签名白名单内 | 三元组未注册仍允许出现在规则或推导结果中 | 防止动作类型越界 |
| F15 | `context_inference.rules` × 推导子语言 | 纯正规则或分层否定 | 递归否定、无界聚合 | 保证可判定与可解释 |
| F16 | `decision_search.pushdown` × 语义判据 | 可证明等价，或满足保守超集并残差收敛 | 无证明直接下推并替代完整求值 | 防止检索语义漂移 |
...
| --- | --- | --- | --- |
| `P0`（阻断） | 结构与语义完整性门禁 | Schema、选择器可解析、动作/关系已注册、动作签名不越界、不可消歧冲突为 0 | 直接阻断发布 |
| `P0`（阻断） | 安全底线门禁 | SoD、基数约束、高敏降级、强制 obligations | 直接阻断发布 |
| `P0`（阻断） | 生命周期完整性门禁 | 主体移除等关键事件处理器存在 | 直接阻断发布 |
| `P0`（阻断） | 检索语义安全门禁 | `Decision Search` 下推可证明等价，或满足保守超集+残差收敛 | 直接阻断发布 |
...
2. `model_meta.status=published` 时，`catalogs.action_catalog/subject_type_catalog/object_type_catalog/subject_relation_type_catalog/object_relation_type_catalog`、`action_signature.tuples`、`relation_signature` 与 `policies.rules` 都必须非空。  
3. 启用 `decision_search.enabled=true` 时，必须声明 `decision_search.pushdown` 安全策略。  
4. `RULE_CONFLICT_UNRESOLVED`、`ACTION_SIGNATURE_MISMATCH`、`SEARCH_PUSHDOWN_UNSAFE` 与 `OBLIGATION_NOT_EXECUTABLE` 归为 `P0` 阻断项，`OBLIGATION_EXECUTION_DEGRADED` 归为 `P2` 复核项。
...
3. 数学抽象与推导边界：[`09_权限模型数学抽象与推导边界.md`](./09_权限模型数学抽象与推导边界.md)。

### 13.14 统一错误码索引（新增）

为便于实现侧“直接抄表”配置校验与门禁联动，以下索引聚焦本轮新增及强相关错误码。
