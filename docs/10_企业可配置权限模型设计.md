...
| `OBLIGATION_NOT_EXECUTABLE` | error | obligations 无法执行（无执行器/无依赖） | 是 | 修复执行链路 |
...
| --- | --- | --- | --- |
| `P0`（阻断） | 结构与语义完整性门禁 | Schema、选择器可解析、动作/关系已注册 | 直接阻断发布 |
...
| `P1`（强复核） | 冲突与不可达门禁 | unresolved conflict、规则不可达比例 | 进入安全管理员复核 |
| `P1`（强复核） | 属性质量门禁 | 来源可信度、freshness、revocation 时效 | 进入数据治理复核 |
| `P2`（复核） | 可执行性门禁 | obligations 执行成功率、依赖可达性 | 进入运维复核 |
...
1. JSON Schema 草案：[`55_权限配置JSON_Schema草案.md`](./55_权限配置JSON_Schema草案.md)。  
2. 发布门禁规则样例：[`56_权限发布门禁规则样例.md`](./56_权限发布门禁规则样例.md)。
