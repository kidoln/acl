...
2. `model_meta.status=published` 时，`catalogs.action_catalog/subject_type_catalog/object_type_catalog/relation_type_catalog`、`action_signature.tuples` 与 `policies.rules` 都必须非空。  
