...
export function executeSubjectRemovedLifecycle(
  ...
  const subjectRelations = input.relations?.subject_relations ?? [];
  const objectRelations = input.relations?.object_relations ?? [];
  const subjectObjectRelations = input.relations?.subject_object_relations ?? [];
