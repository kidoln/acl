...
import expectedFixtureRaw from "./fixtures/department-kb-permissions.expected.json";
import modelFixtureRaw from "./fixtures/department-kb-permissions.model.json";
import setupFixtureRaw from "./fixtures/department-kb-permissions.setup.json";
...
interface FixtureRuleExpectation {
  ...
  object_owner_ref?: string;
  subject_is_owner?: boolean;
  subject_admin_of?: string[];
  object_owner_department?: string;
}
  ...
  if (submitBody.status === "review_required") {
    const review = await app.inject({
      ...
      payload: {
        ...
        reason: "approve for department kb permissions e2e",
...
describe("department kb permissions e2e", () => {
  ...
  it("loads model file and evaluates department-scoped kb permissions", async () => {
    const suffix = nextId("dept_kb_perm_case");
    const namespace = `${setupFixture.namespace_prefix}.${suffix}`;
    const publishId = nextId("pub_dept_kb_perm");
    ...
    const relationListBody =
      listRelations.json() as ControlRelationListResponse;
    ...
    expect(
      relationListBody.items.some(
        (item) =>
          item.from === "user:eng_admin" &&
          item.to === "department:engineering" &&
          item.relation_type === "is_department_admin",
      ),
    ).toBe(true);
    expect(
      relationListBody.items.some(
        (item) =>
          item.from === "user:eng_member_1" &&
          item.to === "department:engineering" &&
    ...
    for (const testCase of setupFixture.decision_inputs) {
      ...
      for (const [ruleId, ruleExpectation] of Object.entries(
        expectedCase.expected_rules,
      )) {
        const rule = ruleById.get(ruleId);
        expect(
          rule,
          `missing inference rule response: ${ruleId}`,
        ).toBeDefined();
        expect(rule?.matched).toBe(ruleExpectation.matched);

        const expectedSubjectValues =
          ruleExpectation.subject_values_contains ?? [];
        expectedSubjectValues.forEach((value) => {
          expect(rule?.subject_values ?? []).toContain(value);
        });

        const expectedObjectValues =
          ruleExpectation.object_values_contains ?? [];
